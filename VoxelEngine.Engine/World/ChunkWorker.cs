...
public sealed class ChunkWorker : IDisposable
{
    ...
    private readonly float _minSkyLightAmbient;
        ...
        int? workerCount = null)
    {
        ...
        _minSkyLightAmbient = Math.Clamp(minSkyLightAmbient, 0f, 1f);
        ...
        while (!ct.IsCancellationRequested)
        {
            if (_jobQueue.TryDequeue(out var job))
            {
                ...
                try
                {
                    ...
                    SkyLightPropagator.RecalculateChunk(_world, chunk);
                    var (ov, oi, cv, ci, tv, ti) = GreedyMeshBuilder.Build(chunk, _world, _generator, _minSkyLightAmbient);
                    if (ct.IsCancellationRequested)
                        continue;
                    _resultQueue.Enqueue(new ChunkResult(job.ChunkX, job.ChunkZ, job.Kind, chunk, ov, oi, cv, ci, tv, ti));
                }
                finally
                {
