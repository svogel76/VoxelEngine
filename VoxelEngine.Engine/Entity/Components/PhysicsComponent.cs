...
/// <summary>
/// Verwaltet Gravitation, Geschwindigkeitsintegration und AABB-Terrain-Kollision für eine Entity.
/// Update() übernimmt die vertikale Physik (Gravitation + Kollision).
/// MoveHorizontally() wird von AIComponent / InputComponent für horizontale Bewegung aufgerufen.
/// </summary>
public sealed class PhysicsComponent : IComponent, IEntityBoundsProvider
{
    ...
    /// <summary>
    /// Horizontale Bewegung mit Kollisionserkennung (für Animal-AI und Player-Input).
    /// </summary>
    ...
    /// <summary>
    /// Vollständiger Player-Bewegungs-Tick inkl. Sprung, Substeps, Step-Up und FlyMode.
    /// </summary>
        ...
        double      deltaTime)
    {
        ...
        {
            verticalVelocity  = 0f; // will be set by caller's JumpVelocity
        ...
        TrackFallDamage(entity, wasOnGroundBefore);
    }
    ...
    private void TrackFallDamage(Entity entity, bool wasOnGroundBefore)
    {
        if (FlyMode) { _wasFalling = false; _fallStartY = entity.InternalPosition.Y; return; }

        if (!IsOnGround && entity.InternalVelocity.Y < 0f)
        {
            if (!_wasFalling) { _fallStartY = entity.InternalPosition.Y; _wasFalling = true; }
        }
    ...
    private void ResolveHorizontalAxis(ref Vector3 position, float delta, CollisionAxis axis, bool canStepUp, ref Vector3 velocity)
    {
        ...
        if (!_terrainCollision.Collides(_world, candidate)) { position = candidate; return; }
    ...
    {
        if (_stepVisualOffsetY >= 0f) { _stepVisualOffsetY = 0f; return; }
    ...
    public static PhysicsComponent FromJson(JsonElement config, global::VoxelEngine.World.World world, Core.EngineSettings settings)
    {
        ...
        return new PhysicsComponent(world, width, height, settings.Gravity, settings.MaxFallSpeed);
    }
}
